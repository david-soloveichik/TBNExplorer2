...
class TestConstraintsFile:
    ...
    def setup_method(self):
        ...
        self.test_tbn = tempfile.NamedTemporaryFile(mode='w', suffix='.tbn', delete=False)
        self.test_tbn.write('\\UNITS: nM\n')
        self.test_tbn.write('a b >M1, 100\n')
        self.test_tbn.write('a* b* >M2, 100\n')
        self.test_tbn.write('c d >M3, 100\n')
        self.test_tbn.write('c* d* >M4, 100\n')
        self.test_tbn.write('a a b b, 50\n')  # Unnamed monomer
        self.test_tbn.write('c c d d, 50\n')  # Another unnamed monomer
        self.test_tbn.close()
        
        # Create corresponding polymat file
        self.test_polymat = tempfile.NamedTemporaryFile(mode='w', suffix='.tbnpolymat', delete=False)
        base_name = Path(self.test_tbn.name).stem
        self.test_polymat_path = Path(self.test_tbn.name).parent / f"{base_name}.tbnpolymat"
        self.test_polymat.close()
    ...
    def test_contains_single_constraint(self):
        ...
        try:
            result = self.run_filter([self.test_tbn.name, '--constraints-file', constraints_file])
    ...
    def test_constraints_file_with_command_line_monomers(self):
        ...
        try:
            result = self.run_filter([self.test_tbn.name, 'M1', '--constraints-file', constraints_file])
            assert result.returncode != 0
    ...
    def test_nonexistent_constraints_file(self):
        ...
        result = self.run_filter([self.test_tbn.name, '--constraints-file', 'nonexistent.txt'])
        assert result.returncode != 0
    ...
    def test_constraints_with_percent_limit(self):
        ...
        try:
            result = self.run_filter([self.test_tbn.name, '--constraints-file', constraints_file, '--percent-limit', '20'])
            assert result.returncode == 0
    ...
    def test_constraints_with_num_limit(self):
        ...
        try:
            result = self.run_filter([self.test_tbn.name, '--constraints-file', constraints_file, '--num', '2'])
            assert result.returncode == 0
