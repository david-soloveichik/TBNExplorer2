...
class TBN:
    ...
    def __init__(
        self, monomers: List[Monomer], binding_site_index: Dict[str, int], concentration_units: Optional[str] = None
    ):
        """
    ...
    def concentrations(self) -> Optional[np.ndarray]:
        ...
        """
        if (
            self._concentrations_molar is None
            and all(m.concentration is not None for m in self.monomers)
            and self.concentration_units is not None
        ):
    ...
    def concentrations_original_units(self) -> Optional[np.ndarray]:
        ...
        """
        if self._concentrations is None and all(m.concentration is not None for m in self.monomers):
    ...
    def check_star_limiting(self) -> Tuple[bool, Optional[str]]:
        ...
        """
        # Use provided concentrations or unit concentrations
        c = self.concentrations if self.concentrations is not None else np.ones(len(self.monomers))
    ...
    def get_augmented_matrix_for_polymer_basis(self) -> Tuple[np.ndarray, int]:
        ...
        for _site_name, site_idx in self.binding_site_index.items():
        ...
        A_prime = np.column_stack([A, *additional_columns]) if additional_columns else A
    ...
    def __str__(self):
        ...
        return f"TBN Model:\n  Binding sites: {n_sites}\n  Monomers: {n_monomers}\n  Has concentrations: {has_conc}"
