...
class PolymerFilter:
        ...
        with open(filename, 'r') as f:
            for line_num, line in enumerate(f, 1):
                ...
                if constraint_type not in ['CONTAINS', 'EXACTLY']:
                    raise ValueError(f"Invalid constraint type '{parts[0]}' on line {line_num}. Must be CONTAINS or EXACTLY.")
    ...
    def _check_polymer_matches_constraint(self, polymer_counts: np.ndarray, constraint_type: str, 
                                         required_monomer_names: List[str]) -> bool:
        """
        ...
        if constraint_type == 'CONTAINS':
            ...
            for i, count in enumerate(polymer_counts):
                if i not in accounted_indices and count > 0:
                    return False
    ...
    def filter_by_constraints_file(self, constraints_file: str, percent_limit: Optional[float] = None, 
                                  max_count: Optional[int] = None) -> List[Tuple[int, np.ndarray, Optional[float], Optional[float]]]:
        """
    ...
    def format_output_with_constraints(self, filtered_polymers: List[Tuple[int, np.ndarray, Optional[float], Optional[float]]], 
                                      constraints_description: str, percent_limit: Optional[float] = None, 
