    ...
    def compute_equilibrium_concentrations(
        ...
        try:
            ...
            result = subprocess.run(
                [self.coffee_path, cfe_path, con_path, "-o", output_path],
                capture_output=True,
                text=True,
                check=False
            )
            ...
            if len(concentrations) != len(polymers):
                raise RuntimeError(
                    f"COFFEE output has {len(concentrations)} concentrations "
                    f"but expected {len(polymers)}"
                )
    ...
    def _parse_coffee_output(self, filepath: str) -> np.ndarray:
        ...
        for val_str in values:
            try:
                ...
                raise RuntimeError(f"Cannot parse concentration value: {val_str}")
