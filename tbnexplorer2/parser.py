...
class TBNParser:
                ...
                if line.startswith("\\UNITS:"):
                    ...
                    try:
                        ...
                        if units not in ["nM", "pM", "uM", "mM", "M"]:
                            raise ValueError(
                                f"Line {line_number}: Invalid units '{units}'. Must be one of: nM, pM, uM, mM, M"
                            )
                    except IndexError as e:
                        raise ValueError(
                            f"Line {line_number}: Invalid \\UNITS format. Expected '\\UNITS: <unit>'"
                        ) from e
                    continue
                ...
                if monomer_data:
                    ...
                    if name:
                        ...
                        if name in ["UNITS", "MATRIX-HASH"]:
                            raise ValueError(f"Line {line_number}: Monomer name '{name}' is a reserved keyword")
                    ...
                    # Create monomer
                    monomer = Monomer(
                        name=name, binding_sites=binding_sites, concentration=concentration, original_line=line
                    )
    ...
    def _aggregate_identical_monomers(monomers: List[Monomer], binding_site_index: Dict[str, int]) -> List[Monomer]:
        ...
        for _, monomer_group in vector_to_monomers.items():
